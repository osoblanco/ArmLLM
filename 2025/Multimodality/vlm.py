...
class VisionLanguageConnector(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            out_size = hidden_size if i < num_layers - 1 else language_hidden_size
            layers.extend(
                [
...
class VisionLanguageModel(nn.Module):
    ...
    def __init__(self, config: TrainingConfig):
        ...
        # Task 2: Freeze language model (optional, can be unfrozen for fine-tuning)
    ...
    def forward(
        ...
        projected_features = self.connector(
            vision_features
        )  # [batch, num_patches, lm_hidden_size]
