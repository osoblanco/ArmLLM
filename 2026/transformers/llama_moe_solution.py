"""LLaMA-style decoder with a Mixture-of-Experts feed-forward — SOLUTION.

This is the reference solution for `llama_moe_exercise.py`. It implements a
small LLaMA-style decoder-only transformer (RMSNorm, RoPE, grouped-query
attention, SwiGLU) where the dense feed-forward block can be swapped for a
sparse Mixture-of-Experts (MoE) layer in the style of Mixtral:
...
Run modes:
  python llama_moe_solution.py test      # quick forward/backward sanity check
  python llama_moe_solution.py train     # train on WikiText-2 (fixed dataset)
  python llama_moe_solution.py eval      # official benchmark perplexity
  python llama_moe_solution.py generate  # greedy decoding from a prompt
"""
...
class RMSNorm(nn.Module):
    ...
    def _norm(self, x):
        # RMS normalization: divide by the root mean square of the features
        return x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)

    def forward(self, x):
        # Normalize in float32 for stability, then scale by the learned weight
        output = self._norm(x.float()).type_as(x)
        return output * self.weight


def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    # Frequencies: 1 / theta^(2i/dim) for each pair of dimensions
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))
    t = torch.arange(end, device=freqs.device)
    # Outer product: angle of rotation for every (position, frequency) pair
    freqs = torch.outer(t, freqs).float()
    # Unit-magnitude complex numbers e^(i * angle), shape [end, dim//2]
...
def apply_rotary_emb(
    ...
    # Pair consecutive feature dimensions into complex numbers
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2))
    xk_ = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))
    # Broadcast freqs_cis over the head dimension: [seq_len, 1, head_dim//2]
    freqs_cis = freqs_cis[:, None, :]
    # Complex multiplication rotates each pair by the position-dependent angle
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # Rotate queries and keys by their position
        xq, xk = apply_rotary_emb(xq, xk, freqs_cis=freqs_cis)

        # Grouped-query attention: each kv head serves n_rep query heads
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        # SwiGLU: gate with silu(xW1), multiply by xW3, project back with W2
...
class MoEFeedForward(nn.Module):
    ...
    A learned router sends every token to its top-k experts. Each expert is
    an ordinary SwiGLU FeedForward. The layer also returns a load-balancing
    auxiliary loss that pushes the router towards using all experts evenly —
    without it, training tends to collapse onto a few experts.
    """
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x = x.view(-1, dim)  # [n_tokens, dim]

        # 1. Router: a probability distribution over experts for every token
        router_logits = self.gate(x)  # [n_tokens, n_experts]
        router_probs = F.softmax(router_logits, dim=-1)

        # 2. Keep only the top-k experts per token, renormalize their weights
        #    so the combination weights sum to 1
        topk_weight, topk_idx = torch.topk(
            router_probs, self.n_experts_per_tok, dim=-1
        )  # both [n_tokens, k]
        topk_weight = topk_weight / topk_weight.sum(dim=-1, keepdim=True)

        # 3. Dispatch: each expert processes only the tokens routed to it,
        #    outputs are combined weighted by the router
        out = torch.zeros_like(x)
        for e, expert in enumerate(self.experts):
            token_idx, slot_idx = torch.where(topk_idx == e)
            if token_idx.numel() == 0:
                continue
            out[token_idx] += topk_weight[token_idx, slot_idx, None] * expert(
                x[token_idx]
            )

        # 4. Load-balancing auxiliary loss (Switch Transformer / Mixtral):
        #      f_e = fraction of routed slots that went to expert e
        #      P_e = mean router probability of expert e
        #      aux = n_experts * sum_e f_e * P_e
        #    equals 1.0 exactly when routing is perfectly uniform, larger when
        #    the router favors a few experts. Gradients flow through P_e.
        dispatch_mask = F.one_hot(topk_idx, self.n_experts).float()  # [n_tokens, k, E]
        tokens_per_expert = dispatch_mask.mean(dim=(0, 1))  # f_e, [n_experts]
        prob_per_expert = router_probs.mean(dim=0)  # P_e, [n_experts]
        aux_loss = self.n_experts * torch.sum(tokens_per_expert * prob_per_expert)

        return out.view(bsz, seqlen, dim), aux_loss
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        # Queries and keys come from the same chunk here (no KV cache), so
        # the diagonal offset is always 1, independent of start_pos.
        mask = None
        if seqlen > 1:
            mask = torch.full(
        ...
        # Sum the auxiliary losses over all MoE layers
...
if __name__ == "__main__":
    # Run with: python llama_moe_solution.py test|train|eval|generate
